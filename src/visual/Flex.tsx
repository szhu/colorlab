...
type FlexProps = {
  x?: FlexAxisSpec;
  y?: FlexAxisSpec;
} & ({ x: FlexAxisSpec; y?: FlexAxisSpec } | { y: FlexAxisSpec; x?: FlexAxisSpec });
...
function parseAxisSpec(value: FlexAxisSpec | undefined): FlexAxisStyle {
  ...
  if (typeString === "keyword") {
    ...
    return { ...DefaultParsedAxisValue, padding: [padding, padding], alignment };
  } else if (typeString === "length keyword length") {
    let [padding, alignment, gap] = tokens;
    return { ...DefaultParsedAxisValue, padding: [padding, padding], alignment, gap };
  } else if (typeString === "length length keyword") {
    let [paddingStart, paddingEnd, alignment] = tokens;
    return { ...DefaultParsedAxisValue, padding: [paddingStart, paddingEnd], alignment };
  } else if (typeString === "length length keyword length") {
    let [paddingStart, paddingEnd, alignment, gap] = tokens;
    return { ...DefaultParsedAxisValue, padding: [paddingStart, paddingEnd], alignment, gap };
    ...
    return { ...DefaultParsedAxisValue, padding: [paddingStart, paddingEnd], gap };
  } else {
